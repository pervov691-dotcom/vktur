...
# ========== КОНФИГУРАЦИЯ ==========
GROUP_TOKEN = "vk1.a.qUyg4SYpR2414W6nHk3hZB4ggpljiji-rBo3P2TBcXlmuEc-jOtXEc_T5BqoKGcIExwbmn5nAyEUTh5NkMMdqVb2qumib8fjaA2JrW1MUBbTyptBks0Khp4QgzvER2gGm9U485X1rnIQ3B3S4lu_BmGFf_tsO6zY5Slr2kC6x5GcKR5C1xzl-CqoTytONqeUyw8RUYys0RQSD7DOaSZQSg"
GROUP_ID = 237951367
ADMIN_IDS = [1024252142]  # ВАШ ID
...
# ========== ФУНКЦИЯ ДЛЯ ПУБЛИКАЦИИ ТОПА ==========
def post_top_to_wall():
    """Публикация ТОП-15 в сообществе"""
    conn = sqlite3.connect('tyaryzhka.db')
    c = conn.cursor()
    ...
    try:
        ...
        print("✅ ТОП опубликован в сообществе")
...
def handle_raid_join(user_id, raid_id):
    ...
    # Если уже в этой стрелке - возвращаем
...
def handle_raid_attack(user_id, weapon_text):
    ...
    # Если нет состояния - пробуем найти рейд по участникам
    ...
    # Если в состоянии нет raid_code, ищем рейд
    if not raid_code:
    ...
    # Проверяем, есть ли игрок в списке участников
